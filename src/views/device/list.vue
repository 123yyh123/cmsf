      ...
      >
        ...
        <el-table-column prop="bindingStatus" label="绑定状态" width="100">
          <template slot-scope="scope">
            <el-tag
                :type="
                scope.row.bindingStatus === '已绑定'
                  ? 'success'
                  : scope.row.bindingStatus === '未绑定'
                  ? 'info'
                  : 'warning'
              "
            >
        ...
        <el-table-column prop="status" label="状态" width="100">
          <template slot-scope="scope">
            <el-tag
                @click="updateDeviceStatus(scope.row)"
                class="bindStatus"
                :type="
                scope.row.status === '正常'
                  ? 'success'
                  : scope.row.status === '维修中'
                  ? 'warning'
                  : scope.row.status === '损坏'
                  ? 'danger'
                  : 'info'
          ...
          <template slot-scope="scope">
            <div style="text-align: center">
              <el-button
                  size="mini"
                  type="text"
                  @click="handleBindDetail(scope.row)"
                  style="color: #409eff"
              >绑定详情
              </el-button>
              <el-button
    ...
    <el-dialog title="编辑设备" :visible.sync="editDialog" width="500px" center>
      ...
      >
        ...
        <el-form-item label="保修到期" prop="warrantyUntil">
          <el-date-picker
              v-model="editForm.warrantyUntil"
              type="date"
              placeholder="选择日期"
              style="width: 100%"
          />
...
export default {
  ...
  methods: {
    ...
    download() {
      this.$confirm('确定要下载设备二维码吗？', '提示', {
        ...
        getAllQRCode().then(res => {
          ...
          link.style.display = 'none';
          link.href = url;
          link.setAttribute('download', '设备二维码数据.zip');
          document.body.appendChild(link);
          link.click();
          loading.close();
        }).catch(() => {
    ...
    submitEditStatusForm() {
      updateDeviceBindStatus(this.editStatusForm).then((res) => {
        if (res.data.code === 200) {
          this.$message({
            type: "success",
            message: "修改成功",
    ...
    getDeviceList() {
      const params = {
        ...
        "purchaseDate.startDate":
            this.filters.purchaseDate === "" ? "" : this.filters.purchaseDate[0],
        "purchaseDate.endDate":
            this.filters.purchaseDate === "" ? "" : this.filters.purchaseDate[1],
    ...
    handleDelete(row) {
      this.$confirm(
          `确认删除设备 ${row.deviceName} (${row.deviceCode}) 吗？`,
          "提示",
          {
            type: "warning",
          }
