        ...
        <el-table-column prop="repairResult" label="维修结果" width="100">
          <template slot-scope="scope">
            <el-tag
                ...
                class="bindStatus"
                :type="
                scope.row.repairResult === '正常'
                  ? 'success'
                  : scope.row.repairResult === '维修中'
                  ? 'warning'
                  : scope.row.repairResult === '损坏'
                  ? 'danger'
                  : 'info'
              "
            >
...
<script>
import {getRepairDeviceBySelf, updateDeviceRepairStatus} from "@/apis/device";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getRepairDeviceBySelf(params).then(res => {
        if (res.data.code === 200) {
